...
from minimax_alpha_beta_h_nic_nn import TimeUp, minimax
from nn_heuristic import NNHeuristic, extract_features
from reversi import reversi
from utils import apply_move, get_legal_moves

# ── NN Heuristic Setup ───────────────────────────────────────────────────────
WEIGHTS_PATH = os.path.join(
    os.path.dirname(__file__), "../src", "weights", "heuristic_v1.npz"
)
...
def play_game(depth, white_heuristic, black_heuristic, hscore_heuristic, rng):
    ...
    while True:
        ...
        use_random = move_number < RANDOM_OPENING_MOVES or rng.random() < EPSILON
        ...
        if use_random:
            ...
            jittered_depth = max(
                1, depth + rng.integers(-DEPTH_JITTER, DEPTH_JITTER + 1)
            )
...
def _assign_game_modes(num_games, nn_available):
    ...
    modes = [MODE_NN_SELF] * n_nn + [MODE_CH_SELF] * n_ch + [MODE_CROSS] * n_cross
...
def main():
    ...
    print(
        f"  Game mix: NN self-play={mode_counts.get(MODE_NN_SELF, 0)}, "
